...
public class RemotePgToLocalPgFdwWorkflow implements Workflow {
    ...
    public WorkflowExecutionResult execute(WorkflowJobSnapshot snapshot, WorkflowParams params) {
        try {
            ...
            String stagingTable = copyViaFdw(snapshot, params);
    ...
    private void verifyForeignTable(WorkflowJobSnapshot snapshot) {
        ...
        log.info("Verifying foreign table: {}", foreignTable);
        // 단순히 LIMIT 1 쿼리로 연결성 검증
        String verifySql = "SELECT 1 FROM " + foreignTable + " LIMIT 1";
        try {
            namedJdbcTemplate.getJdbcTemplate().execute(verifySql);
            log.info("Foreign table {} is accessible.", foreignTable);
        } catch (Exception e) {
            throw new RuntimeException("Failed to verify foreign table access for " + foreignTable, e);
        }
    }
