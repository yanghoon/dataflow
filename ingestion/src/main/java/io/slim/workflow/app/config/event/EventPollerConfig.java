...
public class EventPollerConfig implements SchedulingConfigurer {
    ...
    public JdbcEventMessagePoller unifiedPoller() {
        String extractSql = "SELECT * FROM outbox_event WHERE status = 'READY' AND (extensions->>'next_retry_at' IS NULL OR extensions->>'next_retry_at' <= to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS.US\"Z\"')) FOR UPDATE SKIP LOCKED";
        String updateSql = "UPDATE outbox_event SET status = 'DONE', updated_at = CURRENT_TIMESTAMP WHERE id IN (:ids)";
        return new JdbcEventMessagePoller(repository, eventPublisher, extractSql, updateSql, "Unified-Poller", maxAttempts, initialInterval);
    }
